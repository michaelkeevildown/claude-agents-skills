...
- Change `assert!(result.is_err())` to `assert!(matches!(result, Err(AuthError::InvalidCredentials)))`
- Change `assert!(result.is_ok())` to destructure and check specific fields
- Fix `use` paths to match the module structure implied by `affected-files`
- Fix expected types to match the feature doc's specification (`Result<T, E>` vs `Option<T>`)
...
If a test fails due to a test-side error (compile error in the test file itself), fix it.
